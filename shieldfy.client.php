...
class ShieldfyAPIConnector
{
    ...
    private function setHash()
    {
        ...
        $hash = hash_hmac('sha256', $this->data, $this->secret);
...
class ShieldfyCoreShield{
    ...
    public function report($info , $judgment, $block = false)
    {
        ...
        $judgment['rulesIds'] = [];
        foreach($judgment['infection'] as $infected){
            $judgment['rulesIds'] = array_merge($judgment['rulesIds'], $infected);
        }
        ...
        $res = $this->callApi('session/threat',$data);
        if($block){
            $this->block();
        }
    }
...
class ShieldfyShield extends ShieldfyCoreShield{
	...
	public function shield(){
        ...
        if(!file_exists($session_cache_file)){
            ...
            $result = $this->callApi("session/start",array(
                'host' => $_SERVER['HTTP_HOST'],
                'sessionId' => $this->sessionID,
                'user' => array(
                    'id'        => $userID,
            ...
            $result = $this->callApi("session/step",
                array(
                    'sessionId'=> $this->sessionID,
                    'host'=> $_SERVER['HTTP_HOST'] , 
                    'info'=> array(
                        'code'=>http_response_code(),'time'=>time(), 
	...
	private function run(){
        ...
        if(SHIELDFY_HOST_ADMIN != '' && strpos($info['uri'],SHIELDFY_HOST_ADMIN) === 1){
           return;
        }
